...
def main():
    ...
    logger.info("Starting aw-watcher-steam")
    ...
    while True:
        ...
        try:
            if game_data:
                ...
                logger.info(f"Currently playing {currently_playing_game}")
            else:
                logger.info("Currently not playing any game")
        except Exception as e:
            logger.error("Error sending data" + str(e))
            logger.error(traceback.format_exc())
        sleep(poll_time)
